...
# Agent data storage file
AGENT_DATA_FILE = '/app/data/agent_identity.json'
...
class DIOAgent:
    def __init__(self, config: Dict[str, Any]):
        ...
        # Track last normal range log time
        self.last_normal_log_time = 0
        
        # Load or create persistent agent identity
        self.agent_identity = self._load_agent_identity()
        self.agent_id = self.agent_identity['id']
        hostname = self.agent_identity['hostname']
        self.name = config.get('name', f'Agent-{hostname}')
        self.hostname = hostname
        self.ip_address = self.agent_identity['ip_address']
        self.os_type = self.agent_identity['os_type']
        self.rank = 0
        self.status = 'offline'
        self.threats_detected = 0
        
        # Ensure data directory exists
        os.makedirs(os.path.dirname(AGENT_DATA_FILE), exist_ok=True)
    
    def _load_agent_identity(self) -> Dict[str, str]:
        """Load persistent agent identity or create new one"""
        if os.path.exists(AGENT_DATA_FILE):
            try:
                with open(AGENT_DATA_FILE, 'r') as f:
                    identity = json.load(f)
                logger.info(f"Loaded persistent agent identity: {identity['id']}")
                return identity
            except Exception as e:
                logger.error(f"Error loading agent identity: {e}")
        
        # Create deterministic identity if none exists or if file is corrupted
        import hashlib
        agent_hostname = socket.gethostname()  # Define hostname variable
        deterministic_id = hashlib.md5(agent_hostname.encode()).hexdigest()[:16]
        agent_id = f"agent-{deterministic_id}"
        ...
        self._save_agent_identity(new_identity)
        logger.info(f"Created/loaded deterministic agent identity: {agent_id} (hostname: {agent_hostname})")
        return new_identity
    
    def _save_agent_identity(self, identity: Dict[str, str]):
        """Save agent identity to persistent storage"""
        try:
            with open(AGENT_DATA_FILE, 'w') as f:
                json.dump(identity, f, indent=2)
            logger.info(f"Saved agent identity: {identity['id']}")
        except Exception as e:
            logger.error(f"Error saving agent identity: {e}")
    ...
    def detect_anomalies(self, metrics: Dict[str, Any]) -> list:
        ...
        if not anomalies:
            # Only log normal range message once per hour to reduce log spam
            current_time = time.time()
            if current_time - self.last_normal_log_time >= 3600:  # 1 hour = 3600 seconds
    ...
    async def report_threat(self, anomaly: Dict[str, Any], metrics: Dict[str, Any]) -> bool:
        ...
        try:
            threat_data = {
                ...
                'name': f"{anomaly['type'].replace('_', ' ').title()} on Agent {self.agent_id}",
                'type': anomaly['type'],
                ...
                'metrics': metrics,
                'timestamp': datetime.now().isoformat(),
                # Include system info for proper display in UI
                'system_info': {
                    'hostname': self.hostname,
                    'ip_address': self.ip_address,
                    'os_type': self.os_type
        ...
        while True:
            try:
                ...
                for anomaly in anomalies:
                    ...
                    await self.report_threat(anomaly, metrics)
    ...
    async def start(self):
        ...
        logger.info(f"💻 OS Type: {self.os_type}")
        logger.info(f"🔗 Nerve Center: {self.nerve_center_url}")

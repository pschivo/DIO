...
async def system_health_monitor():
    ...
    while True:
        try:
            ...
            active_threats = len([t for t in threats_cache.values() if t.status == "active"])
...
async def register_agent(agent_data: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    try:
        ...
        threats[agent_id] = []        # Initialize with empty list
        agent_data = {
            'id': agent.id,
            'name': agent.name,
...
async def simulate_attack_on_agent(agent_id: str, attack_data: Dict[str, Any]):
    ...
    try:
        ...
        if attack_data.get('attack_simulation') and agent_id in agents:
    ...
    try:
        ...
        event_data = {
            ...
            'details': {
                ...
                'agent_info': {
                    'hostname': agents_cache[agent_id].hostname if agent_id in agents else f'host-{agent_id[:8]}',
                    'os_type': agents_cache[agent_id].osType if agent_id in agents else 'Linux',
                    'ip_address': agents_cache[agent_id].ipAddress if agent_id in agents else '172.20.0.1'
                },
...
async def get_threats():
    ...
    return list(threats_cache.values())
...
async def create_threat(threat_data: Dict[str, Any]):
    ...
    try:
        ...
        if agent_id and agent_id in agents:
            ...
            agents_cache[agent_id].threats += 1
            logger.info(f"Updated threat count for agent {agent_id}: {agents_cache[agent_id].threats}")
        else:
            agent_info_data = {

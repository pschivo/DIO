...
import psutil
import socket
from database import db_manager, Base, Agent
from database import db_manager, Base, Agent, Threat, Evidence
...
# In-memory cache for performance (backup to database)
...
async def process_agent_data(agent_data: Dict[str, Any]):
    ...
    try:
        logger.info(f"🧠 Processing data from agent: {agent_data.get('agent_id', 'unknown')}")
        
        # Store raw data in evidence store
        agent_id = agent_data.get('agent_id')
        if agent_id:
            if agent_id not in evidence_store:
                evidence_store[agent_id] = []
            evidence_store[agent_id].append({
                'timestamp': datetime.utcnow(),
                'data': agent_data
            })
            
        # Process telemetry, metrics, and detect anomalies
...
async def backup_database():
    """Backup critical database data before restart"""
    try:
        ...
        logger.info(f"Backed up {len(agents)} agents")
        
        # Backup threats
        threats = session.query(Threat).all()
        for threat in threats:
            backup_data['threats'].append({
                'id': threat.id,
                'name': threat.name,
                'type': threat.type,
                ...
                'resolvedAt': threat.resolvedAt.isoformat() if threat.resolvedAt else None
            })
        logger.info(f"Backed up {len(threats)} threats")
        ...
        logger.info(f"Backed up {len(evidence)} evidence records")
        
        session.close()
        
        # Save backup to file
        backup_file = f"/app/backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    try:
        ...
        for agent_data in backup_data.get('agents', []):
            agent = Agent(
                ...
                threats=agent_data['threats'],
                ipAddress=agent_data['ipAddress'],
                osType=agent_data['osType'],
                version=agent_data.get('version', '1.0.0'),
                createdAt=datetime.fromisoformat(agent_data['createdAt']) if agent_data.get('createdAt') else datetime.now(timezone.utc),
                updatedAt=datetime.fromisoformat(agent_data['updatedAt']) if agent_data.get('updatedAt') else datetime.now(timezone.utc)
            )
            session.add(agent)
        
        # Restore threats
        for threat_data in backup_data.get('threats', []):
            threat = Threat(
                id=threat_data['id'],
                ...
                detectedAt=datetime.fromisoformat(threat_data['detectedAt']) if threat_data.get('detectedAt') else datetime.now(timezone.utc),
                resolvedAt=datetime.fromisoformat(threat_data['resolvedAt']) if threat_data.get('resolvedAt') else None
            )
            session.add(threat)
        ...
        for event_data in backup_data.get('events', []):
            event = Event(
                ...
                createdAt=datetime.fromisoformat(event_data['createdAt']) if event_data.get('createdAt') else datetime.now(timezone.utc),
                updatedAt=datetime.fromisoformat(event_data['updatedAt']) if event_data.get('updatedAt') else datetime.now(timezone.utc)
            )
            session.add(event)
        
        # Restore evidence
        for ev_data in backup_data.get('evidence', []):
            evidence = Evidence(
                id=ev_data['id'],
                agentId=ev_data['agentId'],
                type=ev_data['type'],
                severity=ev_data['severity'],
                title=ev_data['title'],
                description=ev_data['description'],
                rawData=ev_data['rawData'],
                status=ev_data['status'],
                confidence=ev_data['confidence'],
                timestamp=datetime.fromisoformat(ev_data['timestamp']) if ev_data.get('timestamp') else datetime.now(timezone.utc)
            )
        ...
        logger.info(f"   - Restored {len(backup_data.get('agents', []))} agents")
        logger.info(f"   - Restored {len(backup_data.get('threats', []))} threats")
        logger.info(f"   - Restored {len(backup_data.get('events', []))} events")
        logger.info(f"   - Restored {len(backup_data.get('evidence', []))} evidence records")
    ...
    while True:
        try:
            ...
            if learning_data:
                ...
                #             agents_cache[agent_id].rank = min(4, agents_cache[agent_id].rank + 1)
                            
        except Exception as e:
            logger.error(f"Error in federated learning cycle: {e}")

async def initialize_caches():
    """Initialize all cache dictionaries to prevent undefined variable errors"""
...
async def health_check():
    ...
    # Get agents from database for health check
    try:
        all_agents = db_manager.get_all_agents()
        active_agents = len([a for a in all_agents if a.status == "active"])
        threats_count = len([t for t in db_manager.get_all_threats() if t.status == "active"])
    except Exception as e:
        logger.error(f"Error getting agents for health check: {e}")
        all_agents = []
        active_agents = 0
        threats_count = 0
    ...
    return {
        ...
        "agents_connected": active_agents,
        "threats_active": threats_count,
        "database": db_health,
        "timestamp": datetime.now().isoformat()
    }
...
async def register_agent(agent_data: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    try:
        ...
        metrics[agent_id] = []  # Initialize with empty list
        agent_data = {
            'id': agent.id,
            'name': agent.name,
            'hostname': agent.hostname,
...
async def update_agent_metrics(agent_id: str, metrics_data: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Update agent info
        agents_cache[agent_id].cpu = metrics_data.get('cpu', 0)
        agents_cache[agent_id].memory = metrics_data.get('memory', 0)
        agents_cache[agent_id].lastSeen = datetime.now(timezone.utc).isoformat()
...
@app.post("/agents/{agent_id}/simulate-attack")
async def simulate_attack_on_agent(agent_id: str, attack_data: Dict[str, Any]):
    """Receive attack simulation data and forward to agent for detection"""
    try:
        logger.info(f"🎯 Received attack simulation for agent {agent_id}: {attack_data.get('attack_type', 'unknown')}")
        
        # In a real system, this would forward the attack simulation to the actual agent
        # For now, we'll create a simulated threat detection to demonstrate the concept
        if attack_data.get('attack_simulation') and agent_id in agents_cache:
            attack_type = attack_data.get('attack_type', 'unknown')
            intensity = attack_data.get('intensity', 0.8)
            
            # Simulate the agent detecting the attack
            if attack_type == 'cpu_exhaustion':
                await simulate_agent_threat_detection(
                    agent_id, 
                    'cpu_anomaly',
                    'high',
                    f"CPU exhaustion attack detected - simulated intensity: {intensity:.1f}",
                    intensity
                )
                ...
                await simulate_agent_threat_detection(
                    ...
                    'memory_anomaly', 
                    'high',
                    f"Memory leak attack detected - simulated intensity: {intensity:.1f}",
                    intensity
                )
                ...
                await simulate_agent_threat_detection(
                    ...
                    'network_anomaly',
                    'critical', 
                    f"Network flood attack detected - simulated intensity: {intensity:.1f}",
                    intensity
                )
            else:
                await simulate_agent_threat_detection(
        ...
        return {"success": True, "message": "Attack simulation forwarded to agent"}
    except Exception as e:
        logger.error(f"Error simulating attack on agent {agent_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to simulate attack")

async def simulate_agent_threat_detection(agent_id: str, threat_type: str, severity: str, description: str, confidence: float):
    """Simulate an agent detecting a threat and reporting it"""
    try:
        ...
        threat = Threat(
            id=threat_id,
            name=f"{threat_type.replace('_', ' ').title()} on Agent {agent_id[:8]}",
            type=threat_type,
            severity=severity,
        ...
        # Increment agent threat count
        if agent_id in agents_cache:
            agents_cache[agent_id].threats += 1
            logger.info(f"🚨 Agent {agent_id[:8]} detected {threat_type} (threats: {agents_cache[agent_id].threats})")
        
        # Create event for the threat
        event_data = {
            ...
            'details': {
                ...
                'agent_info': {
                    'hostname': agents_cache[agent_id].hostname if agent_id in agents_cache else f'host-{agent_id[:8]}',
                    'os_type': agents_cache[agent_id].osType if agent_id in agents_cache else 'Linux',
                    'ip_address': agents_cache[agent_id].ipAddress if agent_id in agents_cache else '172.20.0.1'
                },
                'description': description,  # Add description here for Events tab
                'confidence': confidence
            }
        }
        ...
        if db_event_id:
            logger.info(f"✅ Attack simulation threat event saved to database: {db_event_id}")
        else:
            logger.warning("⚠️ Failed to save attack simulation threat event to database")
        
        logger.info(f"🎯 Simulated threat detection: {threat.name} (confidence: {confidence:.2f})")
        
    except Exception as e:
        logger.error(f"Error simulating threat detection: {e}")
...
async def create_threat(threat_data: Dict[str, Any]):
    ...
    try:
        ...
        agent_info_data = {}
        if agent_id and agent_id in agents_cache:
            agent = agents_cache[agent_id]
            agent_info_data = {
                'hostname': agent.hostname,
            ...
            logger.info(f"✅ Using cached agent info for threat: {agent_id} -> {agent.hostname}")
            # Update agent threat count
            agents_cache[agent_id].threats += 1
            logger.info(f"🔍 DEBUG: Updated threat count for agent {agent_id}: {agents_cache[agent_id].threats}")
        else:
            # Try to get agent info from database as fallback
            try:
                db_agents = db_manager.get_all_agents()
                agent_from_db = next((a for a in db_agents if a.id == agent_id), None)
                if agent_from_db:
                    agent_info_data = {
                        'hostname': agent_from_db.hostname,
                        'os_type': agent_from_db.os_type,
                        'ip_address': agent_from_db.ip_address
                    }
                    logger.info(f"Retrieved agent info from database for {agent_id}: {agent_from_db.hostname}")
                else:
                    # Create minimal agent info for unknown agent
                    agent_info_data = {
                        'hostname': f"agent-{agent_id[:8] if agent_id else 'unknown'}",
                        'os_type': 'Unknown',
                        'ip_address': 'Unknown'
                    }
                    logger.warning(f"⚠️ Agent {agent_id} not found in cache or database, using minimal info")
            except Exception as e:
                logger.error(f"Error retrieving agent info from database: {e}")
...
async def create_evidence(evidence_data: Dict[str, Any]):
    ...
    try:
        ...
        # Get real agent info if available
        if evidence_data.get('agent_id') and evidence_data.get('agent_id') in agents_cache:
            agent = agents_cache[evidence_data.get('agent_id')]
            metrics_data = {
                'hostname': agent.hostname,
                'os_type': agent.osType,
                'ip_address': agent.ipAddress,
            ...
            agent_id = evidence_data.get('agent_id', 'unknown')
            metrics_data = {
                'hostname': f"agent-{agent_id[:8] if agent_id else 'unknown'}",
                'os_type': 'Linux',
    ...
    try:
        ...
        if event_id.startswith("evidence-"):
            ...
            if actual_id in threats:
                ...
                if agent_id and agent_id in agents_cache:
                    agents_cache[agent_id].threats = max(0, agents_cache[agent_id].threats - 1)
                    logger.info(f"Decremented threat count for agent {agent_id}: {agents_cache[agent_id].threats} threats remaining")

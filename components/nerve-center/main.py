...
async def simulate_agent_threat_detection(agent_id: str, threat_type: str, severity: str, description: str, confidence: float):
    ...
    try:
        ...
        if agent_id in agents:
            ...
            logger.info(f"🚨 Agent {agent_id[:8]} detected {threat_type} (threats: {agents[agent_id].threats})")
        ...
        event_data = {
            ...
            'details': {
                ...
                'agent_info': {
                    'hostname': agents[agent_id].hostname if agent_id in agents else f'host-{agent_id[:8]}',
                    'os_type': agents[agent_id].osType if agent_id in agents else 'Linux',
                    'ip_address': agents[agent_id].ipAddress if agent_id in agents else '172.20.0.1'
                },

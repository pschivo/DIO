...
export default function DIODashboard() {
  ...
  useEffect(() => {
    ...
    fetchNetworkMetrics()
    const interval = setInterval(fetchNetworkMetrics, 10000) // Update every 10 seconds
    return () => clearInterval(interval)
  }, [])
    ...
    try {
      ...
      if (result.success) {
        ...
        setThreats(prev => prev.map(threat => 
          threat.id === eventId.replace('threat-', '') ? { ...threat, status: 'acknowledged' } : threat
        ))
      }
    } catch (error) {
                            ...
                            <div className="flex space-x-2">
                              {threat.status !== 'acknowledged' && (
                                <Button 
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    acknowledgeEvent(`threat-${threat.id}`)
                                  }}
                                >
                                  Acknowledge
                                </Button>
                ...
                <Card key={component.component} className={isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-card border-border'}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-muted-foreground'}`}>
                      <p>Uptime: {Math.floor(component.uptime / 3600)}h</p>
                    </div>
                  </CardContent>
                </Card>
              ))}
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>Datetime</TableHead>
                        ...
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>Agent</TableHead>
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>Hostname</TableHead>
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>IP</TableHead>
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>Type</TableHead>
                        <TableHead className={isDarkMode ? 'text-gray-300' : ''}>O.S.</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {events.slice(0, 10).map((event, index) => (
                        <TableRow key={event.id} id={`event-${event.id}`} className="border-b">
